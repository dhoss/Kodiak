...
describe Gallery do
  ...
  it "adds photos to a gallery" do 
    Gallery.create(name: "Fart Knocker", photos:  FactoryGirl.create_list(:attachment, 3))
    expect(Gallery.find_by(name: "Fart Knocker").photos.count).to eq(3)
  end

  it "retrieves the correct photos for a gallery" do
    gallery = Gallery.create(name: "Fart Knocker", photos:  FactoryGirl.create_list(:attachment, 3))
    expect(gallery.photos.pluck(:gallery_id).find{|id| id == gallery.id}).to_not be_nil
  end

  it "moves photos from one gallery to another" do
    gallery1 = Gallery.create(name: "Fart Knocker", photos:  FactoryGirl.create_list(:attachment, 3))
    gallery2 = Gallery.create(name: "Fart Knocker", photos:  FactoryGirl.create_list(:attachment, 3))
